-- Frontend UX expansion: richer profile details, scheduling, and feed activity.

ALTER TABLE profiles
  ADD COLUMN IF NOT EXISTS hip_size text,
  ADD COLUMN IF NOT EXISTS bust_size text,
  ADD COLUMN IF NOT EXISTS bra_cup_size text,
...
CREATE INDEX IF NOT EXISTS listings_provider_scheduled_idx
  ON listings (provider_id, scheduled_at)
  WHERE scheduled_at IS NOT NULL;

CREATE INDEX IF NOT EXISTS status_updates_provider_scheduled_idx
  ON status_updates (provider_id, scheduled_at)
  WHERE scheduled_at IS NOT NULL;
