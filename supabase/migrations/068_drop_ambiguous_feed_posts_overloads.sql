-- Drop every historical get_feed_posts overload so Supabase RPC has one match.
-- The app sends named JSON args; if both character and text variants exist,
-- PostgREST cannot infer the best candidate when p_country_code is null.

DROP FUNCTION IF EXISTS public.get_feed_posts(character, text, integer, integer, integer);
DROP FUNCTION IF EXISTS public.get_feed_posts(character(2), text, integer, integer, integer);
...
AS $$
  WITH feed AS (
    ...
    WHERE su.post_type = 'post'
      ...
      AND (p_country_code IS NULL OR su.country_code::text = p_country_code)
