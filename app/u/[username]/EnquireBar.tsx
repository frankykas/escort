...
export function EnquireBar({ username, isOwnProfile }: Props) {
            ...
            >
              ...
              <div className="flex items-center justify-between py-4 border-b border-white/5">
                <div>
                  ...
                  <p className="mt-0.5 text-[12px] text-zinc-500">How would you like to contact @{username}?</p>
                </div>
                <button
                  onClick={() => setSheetOpen(false)}
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-zinc-800 text-zinc-400 hover:text-zinc-200"
                >
                  <X size={14} />
                </button>
              </div>
              ...
              <div className="space-y-2.5 py-4">
                {[
                  ...
                  },
                  {
                    ...
                    sub: "Message within Cleopatra",
                    color: "text-amber-400",
                    iconBg: "bg-amber-400/15",
                    border: "border-amber-400/15",
                    glow: "hover:shadow-[0_0_20px_rgba(251,191,36,0.1)]",
                  },
                ].map(({ icon: Icon, label, sub, color, iconBg, border, glow }) => (
                  <button
                    key={label}
                    className={`flex w-full items-center gap-4 rounded-2xl border ${border} bg-zinc-900/60 px-4 py-4 text-left transition-all ${glow} active:scale-[0.99]`}
                  >
