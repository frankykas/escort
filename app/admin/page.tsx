    ...
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <Link
            ...
            className="text-[13px] font-semibold text-slate-800 hover:text-pink-500 truncate"
          >
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          ...
          disabled={acting}
          className="rounded-lg bg-emerald-500/15 px-3 py-1.5 text-[11px] font-semibold text-emerald-500 transition hover:bg-emerald-500/25 disabled:opacity-40"
        >
          Approve
        </button>
        <button
          ...
          disabled={acting}
          className="rounded-lg bg-red-500/15 px-3 py-1.5 text-[11px] font-semibold text-red-500 transition hover:bg-red-500/25 disabled:opacity-40"
        >
          Reject
        </button>
      </div>
...
export default function AdminDashboard() {
  ...
  const statCards: {
    ...
    { label: "Providers", value: stats.totalProviders, icon: Crown, color: "text-pink-500", bg: "bg-pink-100" },
    { label: "Clients", value: stats.totalClients, icon: UserCheck, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    ...
    { label: "Pending Verifications", value: stats.pendingVerifications, icon: ShieldCheck, color: "text-pink-500", bg: "bg-pink-100" },
    { label: "Pending Reports", value: stats.pendingReports, icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10" },
  ...
  return (
    <main className="min-h-screen bg-[#fafbfc] text-slate-800">
      ...
      <header className="sticky top-0 z-20 border-b border-gray-200 bg-[#fafbfc]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-[12px] text-slate-500">Cleopatre Platform Overview</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => fetchData(true)}
              disabled={refreshing}
              className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-[12px] font-medium text-slate-500 transition hover:border-gray-300 hover:text-slate-800 disabled:opacity-40"
            >
            ...
            <Link
              href="/"
              className="rounded-lg border border-gray-200 px-4 py-2 text-[13px] font-medium text-slate-500 transition hover:border-gray-300 hover:text-slate-800"
            >
              Back to App
            </Link>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8 space-y-10">
        {/* Stats Grid */}
        <section>
          <h2 className="text-[13px] font-semibold uppercase tracking-widest text-slate-500 mb-4">
            Platform Overview
          </h2>
        ...
        <section>
          <Link
            ...
            className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3.5 transition hover:border-pink-200"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50">
              <ShieldCheck size={18} className="text-amber-500" />
            </div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-800">Explicit content review</p>
              <p className="text-[11px] text-slate-500">Approve or reject pending explicit posts</p>
            </div>
            <span className="text-slate-300">→</span>
          </Link>
          <Link
            href="/admin/bundles"
            className="mt-2 flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3.5 transition hover:border-pink-200"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-pink-50">
              <Crown size={18} className="text-pink-500" />
            </div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-800">All-access bundles</p>
              <p className="text-[11px] text-slate-500">Create and manage platform bundle packages</p>
            </div>
            <span className="text-slate-300">→</span>
          </Link>
        </section>
        ...
        <section>
          <h2 className="text-[13px] font-semibold uppercase tracking-widest text-slate-500 mb-4">
            ...
            {verifications.length > 0 && (
              <span className="ml-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-pink-500 text-[10px] font-bold text-slate-800">
                {verifications.length}
              </span>
            )}
          </h2>
          <div className="rounded-2xl border border-gray-200 bg-gradient-to-b from-white to-gray-50 shadow-md overflow-hidden">
            {verifications.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                <ShieldCheck size={24} className="text-emerald-500/60" />
                <p className="text-[13px] text-slate-400">No pending verifications.</p>
              </div>
            ) : (
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <section>
            ...
            <div className="rounded-2xl border border-gray-200 bg-gradient-to-b from-white to-gray-50 shadow-md overflow-hidden">
              {recentUsers.length === 0 ? (
                <p className="p-6 text-[13px] text-slate-400 text-center">No users yet.</p>
              ) : (
                <div className="divide-y divide-gray-200">
                  {recentUsers.map((u) => (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {u.verification_status === "verified" && (
                            <ShieldCheck size={11} className="text-pink-500 flex-shrink-0" />
                          )}
                        </div>
                        <p className="text-[11px] text-slate-500">
          ...
          <section>
            <h2 className="text-[13px] font-semibold uppercase tracking-widest text-slate-500 mb-4">
              Pending Reports
              {reports.length > 0 && (
                <span className="ml-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-slate-800">
                  {reports.length}
                </span>
              )}
            </h2>
            <div className="rounded-2xl border border-gray-200 bg-gradient-to-b from-white to-gray-50 shadow-md overflow-hidden">
              {reports.length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-10 text-center">
                  <ShieldCheck size={24} className="text-emerald-500/60" />
                  <p className="text-[13px] text-slate-400">No pending reports. All clear.</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-200">
                  {reports.map((report) => (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-slate-500">
                              {report.target_type}
                            </span>
                          </div>
                          {report.details && (
